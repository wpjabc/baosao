...
PlayGame.prototype = {
	...
	*/
	landlord:function (){
		...
		$('body').on('click', '.play_1 .btn .btn_land', function(){		//玩家一叫地主
			...
			$('.score td').eq(0).html($('.score td').eq(0).html()*2);
	...
	*/
	Alert:function (text,num){
		if(num == 1){			//出牌有误的提示，字体红色
			...
			$('.alertNotice').attr('class','alertNotice alerthide');
	...
	landPoker:function (pNum){
		...
		for(var i=0; i<3; i++){
			...
			if(pNum == 2){
				...
				if(screen.width < 1400){
					$('.play_'+pNum+' li:last').css({top:(i*36+510)+'px',left:'96px'});
				}else{
					$('.play_'+pNum+' li:last').css({top:(i*36+560)+'px'});
				}
				// $('.play_'+pNum+' li:last').css({top:(i*30+510)+'px'});
			}else{
				if(screen.width < 1400){
					$('.play_'+pNum+' li:last').css({top:(i*36+510)+'px',right:'90px'});
				}else{
					$('.play_'+pNum+' li:last').css({top:(i*36+560)+'px'});
				}
		...
		if(screen.width > 1500){
			$('.all_poker li').eq(0).animate({transform:'scale(0.6)',left:'-270px'},200).animate({transform:'scale(0.6)',left:'-270px', top:'-358px'},100);
			$('.all_poker li').eq(1).animate({transform:'scale(0.6)',left:'-150px'},200).animate({transform:'scale(0.6)',left:'-150px', top:'-358px'},100);
			$('.all_poker li').eq(2).animate({transform:'scale(0.6)',left:'-30px'},200).animate({transform:'scale(0.6)',left:'-30px', top:'-358px'},100);
			...
			setTimeout(function(){
				...
				for(var i=0; i<that.allPlay[pNum-1].poker.length; i++){
					...
					if(pNum == 2){
						...
						if(screen.width < 1400){
							$('.play_'+pNum+' li:last').css({top:i*30+'px',right:'90px',border:'1px solid #999',borderRadius:'7px'});
						}else{
	...
	onPlay:function (num2){
		...
		if(!that.checkPokers(that.ready_poker.poker)){
			...
			for(var i=0; i<that.allPlay[num2-1].poker.length; i++){
				...
				if(num2 == 2){
					if(screen.width < 1400){
						$('.play_'+num2+' li:last').css({left:36*i+310+'px',borderRadius:'7px',border:'1px solid #999'});
					}else{
	...
	*/
	notPlay:function (num2){
		...
		console.log('当前desktop_poker牌：'+that.desktop_poker.poker);
		console.log('当前desktop_poker牌型：'+that.desktop_poker.type);
		console.log('当前cancle不出次数：'+that.game_status.cancle);
	...
	*/
	integral:function (){
		...
		}else if(that.play_1.role==1 && that.play_2.poker.length == 0 || that.play_1.role==1 && that.play_3.poker.length == 0){		//玩家二或者玩家三出完牌，地主输
			that.play_1.integral = 1000  - parseInt($('.score td').eq(0).html()) * 2;
			that.play_2.integral = 1000  + parseInt($('.score td').eq(0).html());
			that.play_3.integral = 1000  + parseInt($('.score td').eq(0).html());
			$('.score td').eq(1).html(that.play_1.integral);
...
/****************************** 判断牌型的方法 ****************************************/
	...
	aaab:function (cards) {
	    ...
	    if(cards.length != 4) {
	   	...
	   	var arr = this.searchKeys(cards[2], cards)
	...
	aaabb:function (cards) {
	    ...
	    var brr = this.searchKeys(cards[0], cards);

	    var crr = this.searchKeys(cards[4], cards);
	...
	aaabbbcd:function (cards) {
	    ...
	    if(cards.length < 8 || cards.length % 4 != 0) {
	   	...
	   	arr =  this.uniq(cards)
			...
			brr = this.searchKeys(arr[i], cards);
		    console.log(brr)
		    if (brr.length == 3){
	...
	boomBtn:function(){
		...
		setTimeout(function(){
			if(screen.width < 1400){
				...
				$('.audioBoom').append('<audio src="./audio/boom1.mp3" autoplay="true">');
				setTimeout(function(){
					$('.bigBoom').css({transform:'scale(1)',transition:'all 1s ease'});

					// 1000毫秒后消失到原来位置
					setTimeout(function(){
						$('.bigBoom').css({transform:'scale(0)',transition:'all 0s'});
					},1000);
				},1500);
			}
	...
	*/
	airPlane:function(){
		...
		setTimeout(function(){
			...
			$('.aircraft').css({height:'800px'});
			$('.aircraft img').css({left:'-100%',transition:'all 3s ease'});
			
			setTimeout(function(){

				$('.aircraft img').css({top:'10%',left:'100%',transition:'all 0s',zIndex:'999 !important'});

				setTimeout(function(){
				$('.aircraft').css({height:'0px'});
				},1000)
			},2000);
		},1000);
	},
